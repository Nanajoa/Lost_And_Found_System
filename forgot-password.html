...
<head>
  ...
  <title>Forgot Password | Ayera</title>
    ...
    <main class="flex-1 flex items-center justify-center py-10 px-5">
      <div class="w-full max-w-md">
        ...
        <form class="space-y-4" onsubmit="event.preventDefault(); alert('Reset link sent!');">
          <div>
            <label for="email" class="block text-sm font-medium text-[#0e141b]">Email address</label>
            <input type="email" id="email" name="email" required
              class="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-[#2c90e2] focus:ring-[#2c90e2]" 
              placeholder="you@ashesi.edu.gh" pattern="[a-zA-Z0-9._%+-]+@ashesi\.edu\.gh$" />
          </div>
          <button type="submit"
            class="w-full rounded-lg bg-[#2c90e2] text-white font-bold py-2 hover:bg-[#187fd1] transition-all">
            Send Reset Link
          </button>
        </form>
